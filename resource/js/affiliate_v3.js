jQuery(document).ready(function($) {
	...
	// if (lastPart != 'live_jancode' && tracking_id !='' && current_tracking_id == null) {
	// 	window.location = base_url+'?tracking_id='+tracking_id;		
	// }else if (lastPart == 'live_jancode' && tracking_id !='' && current_tracking_id == null) {
	...
	$("#product_keyword3").keyup(function(event) {
		...
		if (key==13) {
			...
			console.log("Search");
			search_by_voice_keyword(keyword)
		}
	});
	...
	var rakutenPromiz = function(barcode) {
	  return new Promise(function(resolve, reject) {
	    ...
	    $.ajax({
	    	// url: 'https://app.rakuten.co.jp/services/api/IchibaItem/Search/20170706?applicationId=1020698445083246210&affiliateId=jafa-sei010000000009&keyword='+barcode,
	    	url: 'https://app.rakuten.co.jp/services/api/IchibaItem/Search/20170706?applicationId=1020698445083246210&sort=%2BitemPrice&trakingId=8484848884&hits=5&affiliateId=13cd26fa.6f864c06.13cd26fb.0eadff3c&keyword='+barcode,
	    	type: 'GET'
	    })
	    .done(function(data) {
	    	// console.log(data);
	...
	var amazonPromiz2 = function(barcode) {
	  return new Promise(function(resolve, reject) {
	...
	var amazon_affiliate_commission11 = 0;
	function get_affiliate_products11(barcode) {
			...
			if (typeof amazonResponse.ItemSearchResponse !== 'undefined' && typeof amazonResponse.ItemSearchResponse.Items.Item !== 'undefined') {
				...
				if (typeof totalItems !=='undefined') {
					...
					if (typeof amazonResponse.ItemSearchResponse.Items.Item[0].SmallImage !=='undefined') {
						small_image = amazonResponse.ItemSearchResponse.Items.Item[1].SmallImage.URL
					}else{
					...
					// console.log(amazonResponse.ItemSearchResponse.Items.Item);
					if (typeof amazonResponse.ItemSearchResponse.Items.Item.ItemAttributes !== 'undefined') {
						productName = toASCII(amazonResponse.ItemSearchResponse.Items.Item.ItemAttributes.Title);						
					}
					...
					if (typeof amazonResponse.ItemSearchResponse.Items.Item.MediumImage !=='undefined') {
						...
						// console.log(amazonResponse.ItemSearchResponse.Items.Item.Item.ImageSets.ImageSet);
						productImage = amazonResponse.ItemSearchResponse.Items.Item.ImageSets.ImageSet.MediumImage.URL;
					}
					
					// var quentity = productName.split('×');
					// console.log("quentity: "+quentity[1].slice(0, 2));
					...
					// console.log(amazonResponse.ItemSearchResponse.Items.Item.Offers.Offer);
					var itemPrice = 0;
		...
		rakutenPromiz(barcode).then(function(data) {
			...
			if (rakuten_data.count>0 && barcode!='49407617') {
				...
				// my_param = my_param
				var moshimo_link = 'http://af.moshimo.com/af/c/click?a_id=1266781&p_id=54&pc_id=54&pl_id=616&s_v='+tracking_id+'&url='+my_param
	...
	function get_affiliate_products(barcode) {
		...
		$(".product_loading_image_mobile").html('<img style="" align="center" src="resource/img/ajax/Spinner-1s-200px.gif">');
		var amazonImage = null;
		amazonPromiz2(barcode).then(function(data) {
			amazonResponse = data;
			$("#product_image").html("&nbsp");
			$(".search_result").text('検索結果');
			$(".product_image_parent").removeClass('d-none').addClass('d-block');
			if (amazonResponse.TotalResultCount>0) {
				...
				if (totalItems > 0) {
					...
					if (typeof amazonResponse.Items[0].Offers.Summaries[0].LowestPrice.PricePerUnit !=='undefined') {
						itemPrice = parseInt(amazonResponse.Items[0].Offers.Summaries[0].LowestPrice.PricePerUnit);
					}else{
					...
					var pricingUrl = amazonResponse.Items[0].DetailPageURL;
					var lowest_price = itemPrice;
					var brsose_nodes = [];
					...
					for (var i = 0; i <totalItems; i++) {
						var checkMin = 0;
						single_price = 0;
						var asin_no = amazonResponse.Items[i].ASIN;
						if (typeof amazonResponse.Items[i].Offers.Summaries !=='undefined') {							
							asin_no = amazonResponse.Items[i].ASIN;
							checkMin = parseInt(amazonResponse.Items[i].Offers.Summaries[0].LowestPrice.Amount);
						}else if(typeof amazonResponse.Items[i].Offers.Listings !== 'undefined') {
							asin_no = amazonResponse.Items[i].ASIN;
							checkMin = parseInt(amazonResponse.Items[i].Offers.Listings[0].Price.Amount);
						...
						if (parseInt(minPrice)>parseInt(checkMin)) {
							...
							if (typeof amazonResponse.Items[i].Offers.Listings !== 'undefined') {
								merchant_name = amazonResponse.Items[i].Offers.Listings[0].MerchantInfo.Name;	
							}else{
								merchant_name = "Amazonマーケットプレイス  ";
							}
							if (merchant_name == 'Amazon.co.jp') {
							...
							minPrice = checkMin;
							if (typeof amazonResponse.Items[i].BrowseNodes !== 'undefined') {
								brsose_nodes = amazonResponse.Items[i].BrowseNodeInfo.BrowseNodes[0];
							}
							if (typeof amazonResponse.Items[i].Images !=='undefined') {
								productImage = amazonResponse.Items[i].Images.Primary.Medium.URL;
								amazonImage = amazonResponse.Items[i].Images.Primary.Medium.URL;
							}
							if (typeof amazonResponse.Items[i].SmallImage !=='undefined') {
								small_image = amazonResponse.Items[i].Images.Primary.Small.URL
							}							
							itemName = amazonResponse.Items[i].ItemInfo.Title.DisplayValue;
							pricingUrl = amazonResponse.Items[i].DetailPageURL;
					...
					get_commission3(brsose_nodes).then(function(data) {
						...
						var totaoPoint = (affiliatePoint+chalinPoint);
						var real_price = (parseInt(minPrice-Math.floor(chalinPoint)));
						amazonRes['shop_name'] = 'アマゾン';
						amazonRes['product_name'] = itemName;
						amazonRes['barcode'] = barcode;
						amazonRes['asin'] = asin_no;
	...
	function set_affiliate_data(response) {
		...
		if (response.length>0) {
			...
			tableHtml += '<tr>';
				...
				tableHtml += '<th style="width: 3%" class="align-middle text-center">サイト</th>';
			tableHtml += '</tr>';
			...
			for (var i = 0; i <3; i++) {					
					...
					if (response[i].product_name !="") {
						...
						tableHtml += '<tr>';
							...
							tableHtml += '<td class="align-middle text-center data_td"> <button class="profite_tabel_btn btn btn-default btn_link btn-lg">'+response[i].shop_name+'</button></td>';
							...
							tableHtml += '</td>';
						tableHtml += '<td class="text-center" style="vertical-align: middle;"><a target="_blank" href="'+response[i].itemUrl+'" attr-btn-link="'+response[i].itemUrl+'" attr-product-name="'+response[i].product_name+'" attr-shop-point="'+shopPoint+'" attr-barcode="'+response[i].barcode+'" attr-shop-name="'+response[i].shop_name+'" attr-product-price="'+productPrice+'" attr-charin-point="'+chalinPoint+'" attr-affiliateRate="'+response[i].affiliateRate+'"  attr-asin="'+response[i].asin+'" class="btn btn-warning purchage_product" id="'+response[i].shop_name+'_affiliate" style="">購入へ</a></td>';
						tableHtml += '</tr>';

						// Mobile Table
						tableHtml2 += '<tr>';

								tableHtml2 += '<td nowrap style="border-left: 0; padding:0;"><a target="_blank" href="'+response[i].itemUrl+'" style="width:100%; padding:10px; display:block;" class="text-center">'+move+'位 <span class="data_td">'+response[i].shop_name+'</span></a></td>';
								...
								// }
								tableHtml2 += '</a></td>';
								tableHtml2 += '<td class="text-center" style="padding:10px"><a target="_blank" href="'+response[i].itemUrl+'" attr-btn-link="'+response[i].itemUrl+'" attr-product-name="'+response[i].product_name+'" attr-shop-point="'+shopPoint+'" attr-barcode="'+response[i].barcode+'" attr-shop-name="'+response[i].shop_name+'" attr-product-price="'+productPrice+'" attr-charin-point="'+chalinPoint+'" attr-affiliateRate="'+response[i].affiliateRate+'"  attr-asin="'+response[i].asin+'" class="btn btn-warning btn-sm purchage_product" id="'+response[i].shop_name+'_affiliate" style="">購入へ</a></td>';
						tableHtml2 += '</tr>';
			...
			if (move==1){
				...
				tableHtml += '<tr>';
					...
					tableHtml += '<td class="text-center" style="padding:10px"><a  class="btn btn-default btn_link btn-lg jafa_btn" id="amazon_itemUrl" style="">購入へ</a></td>';
				tableHtml += '</tr>';
				...
				tableHtml2 += '<tr>';
					...
					tableHtml2 += '<td class="text-center" style="padding:10px"><button class="btn btn-default btn_link btn-lg jafa_btn">購入へ</button></td>';
				tableHtml2 += '</tr>';
			...
			if (move==2){
				...
				tableHtml += '<tr>';
					...
					// }
					tableHtml += '</td>';
					tableHtml += '<td class="text-center"><a attr-btn-link="'+response[0].itemUrl+'" attr-product-name="'+response[0].product_name+'" attr-barcode="'+response[0].barcode+'" attr-shop-name="'+response[0].shop_name+'" attr-product-price="'+productPrice+'" attr-affiliateRate="'+affiliateRate+'" attr-charin-point="'+chalinPoint+'" class="btn btn-warning btn-lg" id="'+response[0].shop_name+'_affiliate">購入へ</a></td>';
				tableHtml += '</tr>';
	...
	var get_commission3 = function(jsonData) {
		var objectDataString = JSON.stringify(jsonData);
	  	return new Promise(function(resolve, reject) {
		    var response;
		    $.ajax({
		    	url: 'main_controller/get_browse_node_commission3/',
		    	type: 'POST',
		    	// dataType: 'json',
		    	data: {node_ids: objectDataString},
		    })
	...
	$(document).on("click",".user_login_btn",function() {
		...
		window.history.pushState(null, null, window.location.href);
		var base_url = $("#base_url").val();
		var tttt = $(this).attr('attr-user-role');
 ...
 //    };
	...
	$("#save_introduce").click(function(event) {
		...
		// return false;
		if (owner_name !=='' && owner_phone !=='' && introduce_name !== '' && introduce_phone !=='') {
			$.ajax({
				url: base_url+'main_controller/save_introduce',
				type: 'POST',
				data: {introduce_name: introduce_name, partner_contact: introduce_phone, partner_email: partner_email},
			})
			.done(function(data) {
				var response = JSON.parse(data);
				var html_data = '';
				...
				// $( ".partner_list_btn" ).trigger( "click" );
				// $(".introduc_success_message").removeClass('d-none').addClass('d-block')
				// $(".introduc_general_message").removeClass('d-block').addClass('d-none')
				// $(".introduc_msg").text(introduce_name+" はあなたのパートナーとして成功しました。");
				// $("#ntroduc_name").text(introduce_name);
				// $("#introduce_sms_link").attr('href', 'sms:'+introduce_phone+'?body='+base_url+'account/?intorid='+data);
	...
	$(document).on('click', '.sent_password_reset_link', function(event) {
		...
		if (forgot_username_email =="") {
			...
			$("#forgot_password_error").text("携帯番号/メールフィールドは必須です");
			$("#forgot_username_email").focus();
			return false
		}
		$.ajax({
			...
			if (response.success==1) {
				...
				html_data += '<tr >';
					html_data+="<td colspan='2' class='text-center'>";
						...
						html_data +='<button class="btn jafa_btn customer_reg_btn  btn-lg" id="register_point_table">新規登録</button>';
					html_data += "</td>"
				html_data += '</tr>';
 ...
 //  	    			}
	...
	function testPromise() {
	    ...
	    p1.then(
	        ...
	        function(val) {
	        	console.log("beforeend with Value");
	        	console.log(val);
	        }).catch(
