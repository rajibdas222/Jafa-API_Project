				...
				<div class="table-responsive-sm">
					<table class="table table-bordered" style="font-size: 26px; border: 3px solid blue;">						
						<thead>
							...
							<tr>
								<th rowspan="2" style="vertical-align: middle;" class="text-center border_4px">加盟企業名</th>
						...
						<div class="form-group">
						  <div class="col-md-12"> 
						    ...
						    <label class="form-check-label" for="date_wise"style="font-size: 20px; width: 90px;">
						    	<input style="height: 20px; width: 20px;" class="form-check-input" id="date_wise" type="radio" name="sorting" value="date_range">
						      日別
						    </label>
		  ...
		  <div class="modal-dialog modal-lg">
		    <div class="modal-content"  style="background: #FFFF99; min-height: 600px; width: 1250px;">
				<div class="modal-header">
					...
					<a id="csv_user_download" class="btn btn-secondary float-right btn-lg" href=""><i class="fa fa-download"></i> ダウンロード会員の情報</a>
	...
	<script type="text/javascript">
		...
		function get_category_company(period = null, end_date = null) {
				...
				$.get(base_url+'main_controller/get_company_category/'+period+'/'+end_date, function(data) {
					...
					if (response.company_summary.length>0) {
						...
						for (var i = 0; i < response.company_summary.length; i++) {
							...
							var total_point = parseInt(response.company_summary[i].temp_point_amount)+parseInt(response.company_summary[i].per_point_amount);
							total_point_amount += total_point;


                            var member_mar =(typeof response.company_summary[i].member_mar == 'NaN') || (response.company_summary[i].member_mar == null) ? 0 : response.company_summary[i].member_mar;

                            var com_mar =(typeof response.company_summary[i].com_mar == 'NaN') || (response.company_summary[i].com_mar == null) ? 0 : response.company_summary[i].com_mar;

							var giving_percentage = parseFloat(member_mar)+parseFloat(com_mar);
							var jafa_percentige = 100-giving_percentage;

							if (response.company_summary[i].major_company == 0) {

							tableHtml += '<tr class="company_member_list" data-link="'+response.company_summary[i].user_id+'">';								
								...
								tableHtml += '<tr class="bg-primary text-light company_member_list" data-link="'+response.company_summary[i].user_id+'">';
								
								tableHtml += '<td class="border_4px"><a class=""  style="text-decoration: underline; color: white;" href="#">'+response.company_summary[i].company_name+'</a></td>';
						...
						tableHtml += '<th  class="text-right border_4px" id="all_purch_total">'+parseInt(total_order_amount).toLocaleString('en')+'</th>';
						tableHtml += '<th class="text-right">'+parseInt(total_point_amount).toLocaleString('en')+'</th>';
						
						tableHtml += '<th class="text-right">'+parseInt(total_temp_point_amount).toLocaleString('en')+'</th>';		
						tableHtml += '<th class="text-right border_4px">'+parseInt(total_per_point_amount).toLocaleString('en')+'</th>';
						tableHtml += '<th class="text-center">-</th>';
						tableHtml += '<th class="text-right">'+parseInt(total_member).toLocaleString('en')+'</th>';	
			...
			$(document).on("click",".company_member_list",function(event) {
				event.preventDefault()
				var user_id = $(this).attr('data-link');
				...
				$("#csv_user_download").attr('href', base_url+'main_controller/download_member_information/'+user_id);
				...
				.done(function(data) {
					...
					if (response.members.length>0) {
						...
						htmlData += '<tr>';
							...
							htmlData += '<th rowspan="2" style=" vertical-align:middle">会員</th>';
							htmlData += '<th colspan="2">商品売上金額</th>';
							htmlData += '<th colspan="2" style="border-right: 6px double black">チャリン<sup>２</sup>P</th>';
						...
						for (var i = 0; i< response.members.length; i++) {
							...
							total_per_jafa += parseInt(perm_point_amount-perm_member_point+perm_company_point);
							...
							htmlData += '<tr>';
								...
								htmlData += '<td class="bg-warning" style="text-align: right; text-align:right;">'+parseInt(temp_point_amount).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-success text-white" style="text-align: right; border-right: 6px double black; text-align:right;">'+parseInt(perm_point_amount).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-warning" style="text-align: right">'+parseInt(temp_company_point).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-success text-white" style="text-align: right">'+parseInt(perm_company_point).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-warning" style="text-align: right">'+parseInt(temp_member_point).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-success text-white" style="text-align: right">'+parseInt(perm_member_point).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-info text-white" style="text-align: right"><a class="giftcode_history text-white" style="font-size:22px;" href="'+base_url+'giftcode_history/'+response.members[i].user_id+'/'+selected_month+'/'+end_date+'">'+parseInt(exchange_amount).toLocaleString('en')+'</a></td>';
								...
								htmlData += '<td class="bg-warning" style="text-align: right">'+parseInt(temp_com_total).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-success text-white" style="text-align: right; border-right: 6px double black">'+parseInt(perm_com_total).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-warning" style="text-align: right">'+parseInt(temp_jafa_point).toLocaleString('en')+'</td>';
								htmlData += '<td class="bg-success text-white" style="text-align: right">'+parseInt(perm_point_amount-perm_member_point+perm_company_point).toLocaleString('en')+'</td>';
							htmlData += '</tr>';
						}
						htmlData += '<tr style="background-color: yellow;">';
						htmlData +='<th class="text-right">計</th>';
						htmlData +='<th class="text-right">'+parseInt(total_temp_order_amount).toLocaleString('en')+'</th>';
						htmlData +='<th class="text-right">'+parseInt(total_per_order_amount).toLocaleString('en')+'</th>';
						htmlData +='<th class="text-right">'+total_temp_point_amount.toLocaleString('en')+'</th>';
						htmlData += '<th style="border-right: 6px double black" class="text-right" id="">'+parseInt(total_per_point_amount).toLocaleString('en')+'</th>';
						htmlData += '<th class="text-right">'+parseInt(total_temp_company).toLocaleString('en')+'</th>';
						...
						htmlData += '<th class="text-right">'+total_per_jafa.toLocaleString('en')+'</th>';
						htmlData += '</tr>';
					}else{
						htmlData += '<tr>';
