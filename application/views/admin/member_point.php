...
<body>
	...
	<div class="container-fluid">
		<div class="row" style="margin-top: 10px;">
			...
			<div class="col-md-6">
				<button style="margin-left: 5px;" type="button" class="btn btn-success float-right btn-lg">購入</button>
				<button style="margin-left: 5px;" type="button" class="btn btn-primary float-right btn-lg">加盟店</button>
				<button style="margin-left: 5px;" type="button" class="btn btn-danger float-right btn-lg">会員</button>
			</div>
		</div>
		<div class="row">
				...
				<div class="table-responsive-sm">
					<table class="table table-bordered table-striped" style="font-size: 26px; border: 3px solid blue;">
						...
						<thead>
							...
							<tr>
								<!-- <th rowspan="2" style="vertical-align: middle;" class="text-center border_4px">会員名</th> -->
						...
						<tbody id="member_list">
							<th colspan="9" style="border-right: 3px solid blue">
	...
	<script type="text/javascript">
		...
		function get_all_member_company(period = null, end_date = null) {
				...
				.done(function(data) {					
					...
					var htmlData = "";
					var total_sealse = +parseInt(temp_order_amount+perm_order_amount);
					...
					if (response.members.length>0) {
						...
						for (var i = 0; i< response.members.length; i++) {
							// Temporary Points
							var member_margin = response.members[i].company_details.member_mar == null? 50:response.members[i].company_details.member_mar;
							...
							// all total
							total_order_amount += parseInt(perm_order_amount+temp_order_amount);
							total_point_amount += parseInt(temp_point_amount+perm_point_amount);

							total_company += parseInt(perm_company_point+temp_company_point);
							total_member += parseInt(perm_member_point+temp_member_point);
							// total_jacos += parseInt(total_point_amount-(total_company+total_member));
							// Total Exchange
							...
							htmlData += '<tr>';
								// htmlData += '<td class="border_4px"><a class="member_purchase_list" href="'+base_url+'purchase_list/'+response.members[i].user_id+'/'+selected_month+'/'+end_date+'">'+response.members[i].fullname+'</a></td>';
								htmlData += '<td class="text-center border_4px">-</td>';
								htmlData += '<td  class="border_4px" style="text-align: right;">'+parseInt(temp_order_amount+perm_order_amount).toLocaleString('en')+'</td>';
								
								htmlData += '<td style="text-align: right;">'+parseInt(temp_point_amount+perm_point_amount).toLocaleString('en')+'</td>';
								htmlData += '<td style="text-align: right">'+parseInt(temp_point_amount).toLocaleString('en')+'</td>';
								htmlData += '<td class="border_4px" style="text-align: right">'+parseInt(perm_point_amount).toLocaleString('en')+'</td>';
								htmlData += '<td style="text-align: right">'+member_margin+'%</td>';
								htmlData += '<td style="text-align: right">'+parseInt(temp_member_point+perm_member_point).toLocaleString('en')+'</td>';
								// htmlData += '<td style="text-align: right">'+perm_member_point+'</td>';
								htmlData += '<td style="text-align: right">'+parseInt(0).toLocaleString('en')+'</td>';

								htmlData += '<td style="text-align: right">'+parseInt((temp_point_amount+perm_point_amount)-(temp_member_point+perm_member_point)).toLocaleString('en')+'</td>';
								
							htmlData += '</tr>';
						}

						htmlData += '<tr style="background-color: yellow;">';
						// htmlData +='<th class="text-center border_4px">-</th>';
						htmlData +='<th class="text-right border_4px">計</th>';
						htmlData +='<th class="text-right border_4px">'+parseInt(total_order_amount).toLocaleString('en')+'</th>';
						htmlData +='<th class="text-right ">'+parseInt(total_point_amount).toLocaleString('en')+'</th>';
						htmlData +='<th class="text-right">'+parseInt(total_temp_point_amount).toLocaleString('en')+'</th>';
						htmlData += '<th class="text-right border_4px" id="">'+parseInt(total_per_point_amount).toLocaleString('en')+'</th>';
						htmlData += '<th class="text-right">-</th>';
						htmlData += '<th class="text-right">'+parseInt(total_member).toLocaleString('en')+'</th>';
						htmlData += '<th class="text-right">'+parseInt(total_company).toLocaleString('en')+'</th>';
						htmlData += '<th class="text-right">'+parseInt(total_point_amount-(total_member+total_company)).toLocaleString('en')+'</th>';
						htmlData += '</tr>';
					}else{
						htmlData += '<tr>';
							htmlData += '<td colspan="11"><center><h3>会員は利用できません。</h3><center></td>';
						htmlData += '</tr>';
					}
			...
			$(document).on("click",".company_member_list",function(event) {
				...
				.done(function(data) {
					...
					var response = JSON.parse(data);
                    console.log(response+" :Company purchase_list");
					// return false;
