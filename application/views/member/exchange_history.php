	...
	<div class="container-fluid" style="margin-top: 20px;">
		<div class="row">
			...
			<div class="col-md-6" style="font-size: 22px;">
				...
				?>
				<span class="float-right">交換履歴</span>
			</div>
			<div class="col-md-6" style="font-size: 28px;">
				
				<a href="<?= base_url() ?>history" class="btn btn-danger btn-lg float-right" onclick="window.close();" id="" style="margin-right: 10px;"><i class="fa fa-user-plus"></i> 戻る</a>
	...
	<script>
		jQuery(document).ready(function($) {
			...
			$("#submit_sorting").click(function(event) {
				...
				var sorting = $("input:checked").val();
				if (sorting == 'all') {
					window.location = base_url+"exchange_history/all/null";
				}else if(sorting=='month_wise'){
					var month_wise = $("#month_wise").val();
					var exploadYear = String(month_wise).split("年");
					var exploadMonth = String(exploadYear[1]).split("月");
					window.location = base_url+"exchange_history/"+exploadYear[0]+'-'+exploadMonth[0]+"/null";
				}else if(sorting=='date_range'){
					...
					if (start_date !="") {
						...
						window.location = base_url+"exchange_history/"+start_year[0]+'-'+start_month[0]+'-'+dateOnly[0]+"/"+end_year[0]+'-'+end_month[0]+'-'+end_dateOnly[0];
