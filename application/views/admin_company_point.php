...
<body">
	<div class="container-fluid" style="margin-top: 20px;">
		<div class="row">
			<div class="col-md-3" style="font-size: 32px;">
				<h2>総括表（ジャコス集計）</h2>
			</div>
			<div class="col-md-2">
				<h1>メイン画面</h1>
			...
			<div class="col-md-7" style="font-size: 28px;">
				...
				<a href="admin_company_point" class="btn btn-info btn-lg float-right" style="margin-right: 10px; margin-top: 10px;">Company Point</a>
				<a href="upload_referal_fee" class="btn btn-info btn-lg float-right" style="margin-right: 10px; margin-top: 10px;">Upload Referal Fee</a>
				<a href="setting_point_sharing" class="btn btn-warning btn-lg float-right" style="margin-right: 10px; margin-top: 10px;">Setting Point Sharing</a>
			...
			<div class="col-md-12" style="margin-top: 20px;">
				<table class="table table-bordered table-striped" style="font-size: 26px; border: 3px solid blue;">					
					...
					<tbody id="company_list">
						<?php
						if (!empty($member_points)) :
							foreach ($member_points as $key => $member_point) :
								
								$company_info = $this->points_model->check_by(array('account_id' => $member_point->parent_id), 'a3m_account_details');
								$company_name = "";
								if (!empty($company_info)) {
									$company_name = $company_info->fullname;
								}
								$type = "Temporary";
								$current_bal = $member_point->temporary_balance;
								if ($member_point->is_temporary == 2) {
									$type = "Permanent";
									$current_bal = $member_point->permanent_balance;
								}
								?>
								<tr>
									<td><?= $member_point->point_tracking ?></td>
									<td><?= $company_name ?></td>
									<td><?= $member_point->fullname ?></td>
									<td><?= $member_point->order_amount ?></td>
									<td><?= $member_point->point_amount ?></td>
									<td><?= $type ?></td>
									<td><?= $current_bal ?></td>
									<td><?= $member_point->updated_at ?></td>
								</tr>
								<?php
							endforeach;
							?>
