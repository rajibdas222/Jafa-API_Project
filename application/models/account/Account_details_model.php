...
class Account_details_model extends CI_Model {
	...
	function get_bank_account_id($account_id)
	{
		return $this->db->get_where('user_bank_info', array('user_id' => $account_id))->row();
	}
	 ...
	 * @return void
	 */
	function update_bank_details($account_id, $attributes = array())
	{
		// Update
		if ($this->get_bank_account_id($account_id))
		{
			$this->db->where('user_id', $account_id);
			$this->db->update('user_bank_info', $attributes);
		}
		// Insert
		else
		{
			$attributes['user_id'] = $account_id;
			$this->db->insert('user_bank_info', $attributes);
		}
	}


	function createAccDetails($user_id,$fullname){
	    $data = array(
	        'account_id' => $user_id,
            'fullname' => $fullname
        );
        $this->db->insert('a3m_account_details', $data );
	}
