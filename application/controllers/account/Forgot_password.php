...
class Forgot_password extends CI_Controller {
	...
	public function ajax_forgot_password()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Username does not exist
			if ( ! $account = $this->account_model->get_by_username_email($this->input->post('forgot_username_email', TRUE)))
			{
				$data['message'] = lang('forgot_password_username_email_does_not_exist');
				$data['success'] = 0;
			}
			// Does not manage password
			elseif ( ! $account->password)
			{
				$data['message'] = lang('forgot_password_does_not_manage_password');
				$data['success'] = 0;
			}
			else
			{
