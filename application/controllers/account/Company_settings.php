...
class Company_settings extends CI_Controller {
	...
	function index()
	{
		...
		$com_id = $this->input->get('tencd');
		// Redirect unauthenticated users to signin page
		if ( ! isset($com_id))
		{
			redirect('account/sign_in/?continue='.urlencode(base_url().'account/company_settings'));
		}
		// Retrieve sign in user

		$data['account'] = $this->username_check($com_id);
		
		$this->authentication->sign_in($data['account']->id, $this->input->post('sign_in_remember', TRUE));
		$data['account'] = $this->account_model->get_by_id($this->session->userdata('account_id'));
		$data['account_details'] = $this->account_details_model->get_by_account_id($data['account']->id);
		...
		$this->load->view('account/company_settings', $data);
	}
	...
	{
	  $account_info = $this->account_model->get_by_username($username);
	  if ($account_info) {
	  	return $account_info;
	  }else{
	  	return false;
